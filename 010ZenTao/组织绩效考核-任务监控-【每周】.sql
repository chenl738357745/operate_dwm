...
, case when finishedByu.realname is null then zt_user.realname else finishedByu.realname end as '实际责任人'
...
,zt_task.estStarted as '预计开始日期'
,zt_task.realStarted as '实际开始日期'
...
-- ,year(zt_task.deadline) '年'
-- ,month(zt_task.deadline) '月'
-- ,week(zt_task.deadline) '周' 
,CONCAT('第',':',week(curdate())-week(STR_TO_DATE('2020-11-13', '%Y-%m-%d')),'周') '当前周' 
,CONCAT('预计第',':',week(zt_task.deadline)-week(STR_TO_DATE('2020-11-13', '%Y-%m-%d')),'周完成',case when week(zt_task.deadline)-week(curdate()) =0 then '(本周)'
else '' end) '完成周'
