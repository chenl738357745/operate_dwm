--获取项目分期树	/REST/http-loadTreeStage.ac	P_MDM_PROJECT_STAGE_TREE_GROUP
--根据ID获得项目信息	/REST/http-loadProject.ac	P_MDM_PROJECT_INFO_GROUP
--获取项目分期楼栋树	/REST/http-loadTreeStageBuilding.ac	P_MDM_EXT_STAGE_BUILDING_TREE
--获得所有项目列表(非前置项目)	/REST/http-loadListProject.ac	P_MDM_PROJECT_LIST_GROUP
--获得所有项目列表(前置项目+非前置项目)	/REST/http-loadListAllProject.ac	P_MDM_PROJECT_ALL_LIST_GROUP
--获得所有分期列表	/REST/http-loadListStage.ac	P_MDM_STAGE_LIST_GROUP
--根据项目ID获取分期列表	/REST/http-loadListProjectStage.ac	P_MDM_PROJECT_STAGE_LIST_GROUP
--根据公司ID获取项目列表	/REST/http-loadListUnitProject.ac	P_MDM_UNIT_PROJECT_LIST_GROUP
--获取项目分期树（包括公司、项目、分期）	/REST/http-loadTreeStage.ac	P_MDM_PROJECT_STAGE_TREE_GROUP
--通过项目id获取项目信息（BI）	/REST/http-loadProjectInfo.ac	 P_MDM_STAGE_INFO_BI_GROUP
--获取项目、分期、楼层树	/REST/http-loadTreeStageBuilding.ac	P_MDM_EXT_STAGE_BUILDING_TREE
--以分页形式返回项目列表	/REST/http-loadPageProject.ac	P_MDM_PROJECT_LIST_PAGE_GROUP
--根据项目id获取标段列表	/REST/http-loadListProjectSection.ac	
--根据所有项目下的分期、标段、楼栋数量	/REST/http-loadProjectInfoNums.ac	
--获取项目、分期、楼栋	/REST/http-listProjectStageBuilding.ac	P_MDM_EXT_STAGE_BUILDING
--获取全部业态	/api/listAllPT	P_MDM_EXT_ALL_PT
--获取楼栋列表	/api/loadBuildingList	P_MDM_EXT_BUILDING_LIST
--获取分期列表	/api/loadStageList	P_MDM_STAGE_LIST_SALE_GROUP
--获取项目列表	/api/loadProjectList	P_MDM_PROJECT_LIST_GROUP_SALE
--根据分期id获取楼栋信息	/api/getBuildingByStage	P_MDM_EXT_BUILDING_BY_STAGE
--根据项目id获取项目和分期信息(提供给销售系统)	/api/getStageTypeByProject	P_MDM_PROJECT_STAGE_SALE_GROUP
--获取楼栋所有房间的信息（新）	/api/saleReportViewByBldguid	P_MDM_EXT_BUILDING_ROOM
--获取项目下的车位&楼栋信息 	/REST/getProjectInfoByProjectId	P_MDM_EXT_PROJECT_CAP_BUILD
--获取分期下的车位&楼栋信息	/REST/getStageInfoByStageId	P_MDM_EXT_STAGE_CAP_BUILD
--获取楼栋所有房间信息	/REST/listRoomByBuildingId	P_MDM_EXT_BUILDING_ROOM_OLD


--------------------------------------------------------
--  文件已创建 - 星期一-三月-22-2021   
--------------------------------------------------------
--------------------------------------------------------
...
begin
    ...
    open info for
        ...
        where ((vIsNoPeriod = 1 and PROJ_ID=vStageId) or PERIOD_ID = vStageId) and BUILD_STATE='10';
end;
...
begin
    open info for
        ...
        with
            ...
            project as (
               ...
               -- left join MDM_PROJ_MAPPING mpm on UPPER(mpm.PROJECT_ORIGINAL_ID) = UPPER(sp.ID)
                inner join unit on UPPER(sp.UNIT_ID) = UPPER(unit."id")
            ),
            -- 分期
            stage as (
                select
            ...
            building as (
                ...
                from SYS_BUILD sb
                inner join stage on UPPER(sb.PERIOD_ID) = UPPER(stage."id")
            )
        select * from unit
        union all
        ...
        left join MDM_PROJ_MAPPING mpm on UPPER(mpm.PROJECT_ORIGINAL_ID) = UPPER(b."projectId");
end;
...
begin
    ...
    open info for
        ...
        where ((vIsNoPeriod = 1 and sp.ID = vStageId) or sps.id = stageGuid)
