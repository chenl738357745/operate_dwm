...
--------------------------------------------------------
  ...
  CREATE OR REPLACE VIEW "V_POM_MONTH_QUARTER_YEAR" ("M_END", "QUARTER_END", "YEAR_END", "M_BEGIN", "QUARTER_BEGIN", "YEAR_BEGIN") AS with 年季月 AS (
    SELECT
        yyyy,
        mm,
        q,
    ...
    WHERE
        lpad(mm, 2, '0') = TO_CHAR(sysdate, 'MM')
        AND lpad(yyyy, 4, '0') = TO_CHAR(sysdate, 'yyyy')
)

select     m_end AS "M_END",
           quarter_end AS "QUARTER_END",
           year_end AS "YEAR_END",
           m_begin AS "M_BEGIN",
           quarter_begin AS "QUARTER_BEGIN", "YEAR_BEGIN" FROM 年季月
;
--------------------------------------------------------
...
(CASE
     WHEN a.actual_end_date IS NOT NULL THEN ''
...
(CASE
     WHEN a.actual_end_date IS NULL THEN ''
...
select
-- 黄伟2020-01-09创建：展示“已审核的计划”的所有未禁用节点的动态情况；涉及到节点的统计、当前状态、预警等都应尽可能从这里取数。
row_number() OVER (ORDER BY a.node_code)                            AS RowCode,
'专项计划'                                                              AS PlAN_Type,              -- 计划类别
b.plan_name                                                         AS Plan_Name,              -- 计划名称
''                                                                  AS Proj_Name,              -- 所属项目
a.node_code                                                         AS Node_Code,              -- 节点编号
a.node_name                                                         AS Node_Name,              -- 节点名称
...
(CASE
     ...
     WHEN a.actual_end_date IS NOT NULL AND TRUNC(actual_end_date) - TRUNC(a.plan_end_date) < 0 THEN '提前完成'
     ELSE '未知' END)                                                 AS Finish_YES_Status_Type, -- 已完成细分类别
a.ACTUAL_BEGIN_DATE,                                                                           -- 实际开始日期
a.actual_end_date,                                                                             -- 实际完成日期
(CASE
     WHEN a.actual_end_date IS NOT NULL THEN ''
     WHEN a.actual_end_date IS NULL AND TRUNC(a.PLAN_END_DATE) - TRUNC(a.plan_end_date) > 0 THEN '预计超期'
     WHEN a.actual_end_date IS NULL AND TRUNC(a.PLAN_END_DATE) - TRUNC(a.plan_end_date) = 0 THEN '预计按时'
     WHEN a.actual_end_date IS NULL AND TRUNC(a.PLAN_END_DATE) - TRUNC(a.plan_end_date) = 0 THEN '预计提前'
     ELSE '无预测' END)                                                AS Estimate_Status,        -- 预测状态
PLAN_START_DATE,                                                                               -- 预计开始日期
PLAN_END_DATE,                                                                                 -- 预计完成日期
a.id,-- 节点ID
PLAN_ID,                                                                                       -- 计划ID
a.template_node_id,                                                                            -- 使用模板节点ID
''                                                                     standard_node_id,       -- 标准节点ID
''                                                                     node_grade,             -- 节点等级(集团级、区域级）
''                                                                     node_level,             -- 任务级别（里程碑、一级节点、二级节点、三级节点）
''                                                                     proj_stage,             -- 项目阶段
''                                                                     main_responsibility,    -- 主责职能
''                                                                     control_type,           -- 控制类型
a.duty_department_id,                                                                          -- 主责部门ID
a.duty_department,                                                                             -- 主责部门
a.IS_DELETE                                                            is_disable,             -- 是否禁用
null                                                                     disable_time,           -- 禁用时间
''                                                                     disable_reason_id,      -- 禁用原因ID
0                                                                     standard_score,         -- 标准分值
''                                                                     reference_date_node_id, --
0                                                                     reference_day,          -- 参考天数
''                                                                     precondition_remark,    -- 前置条件说明
0                                                                     is_contains_permit,     -- 是否证照节点
''                                                                     permit_type,            -- 证照类型
a.completion_standard,                                                                         -- 完成标准说明
a.completion_results_remark,                                                                   -- 完成成果说明
...
a.company_id,                                                                                  -- 公司ID
a.ORIGINAL_NODE_ID
from POM_SPECIAL_PLAN b
         left join POM_SPECIAL_PLAN_NODE a on a.PLAN_ID = b.ORIGINAL_PLAN_ID and b.APPROVAL_STATUS='已审核'
;
--------------------------------------------------------
...
,SUM((CASE    	WHEN TRUNC( A1.ACTUAL_END_DATE) IS NOT NULL AND TRUNC( A1.ACTUAL_END_DATE)  >TRUNC( PLAN_END_DATE)       THEN 		1	ELSE		0 END))   AS "DELAYFINISH"
 ...
 (	SELECT   TRUNC(ADD_MONTHS(M_END,-1)+1)  FROM v_pom_month_quarter_year )  AND
...
WHERE   A.APPROVAL_STATUS='已审核' AND  nvl(A1.IS_DELETE,0)=0  and  nvl(a1.IS_DISABLE,0)=0
...
AND A1. IS_DISABLE=0
 ...
 AND A1.PLAN_END_DATE BETWEEN
		 ...
		 (	SELECT   quarter_begin  FROM v_pom_month_quarter_year )  AND
 (	SELECT quarter_end  FROM v_pom_month_quarter_year)
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE VIEW "V_POM_PROJ_MONITOR_SUMMARYLIST" ("PROJ_ID", "PROJ_NAME", "PROJ_PLAN_ID", "NODE_ID", "PLAN_ID", "PLAN_TYPE", "ORIGINAL_NODE_ID", "WARNING_STATUS", "NODE_CODE", "NODE_NAME", "NODE_LEVEL", "DUTY_DEPARTMENT", "DUTY_DEPARTMENT_ID", "DUTY_MANS", "FINISH_STATUS", "STANDARD_SCORE", "PLAN_START_DATE", "PLAN_END_DATE", "ACTUAL_START_DATE", "ACTUAL_END_DATE", "ESTIMATE_END_DATE", "PROCESS_FEED_BACK_CON", "FINISH_FEED_BACK_CON", "OVERDUE_FEED_BACK_CON", "NODE_FLAG", "ESTIMATE_BEYOND_NODE_FLAG", "COMPLETION_STANDARD", "COMPLETION_RESULTS_REMARK") AS SELECT
     ...
     ,NODEINFO.COMPLETION_STANDARD
     ,NODEINFO.COMPLETION_RESULTS_REMARK
FROM ( SELECT A.PROJ_ID, A.PROJ_NAME, B.PROJ_PLAN_ID, B.ID AS NODE_ID /*任务ID*/,
              A.ID AS PLAN_ID /*计划ID */, A.PLAN_TYPE /*计划类别*/, B.ORIGINAL_NODE_ID /*节点原始ID*/,
              CASE WHEN NODE_LEVEL  in ('里程碑','一级节点') then
                  case /*未到期不亮灯*/
                      WHEN    ACTUAL_END_DATE IS NULL AND  TRUNC( SYSDATE )-trunc(PLAN_END_DATE)<0  THEN ''
 --到期当天内完成反馈
...
TO_CHAR(D.CHARGE_PERSON)
   AS DUTY_MANS,
			 ...
			 B.PLAN_END_DATE, B.ACTUAL_START_DATE AS ACTUAL_START_DATE,
			 B.ACTUAL_END_DATE, B.ESTIMATE_END_DATE AS ESTIMATE_END_DATE,
			 NVL(E.PROCESS_FEED_BACK_CON,0) AS PROCESS_FEED_BACK_CON, 
             NVL(E.FINISH_FEED_BACK_CON,0) AS FINISH_FEED_BACK_CON,
			 NVL(E.OVERDUE_FEED_BACK_CON,0) AS OVERDUE_FEED_BACK_CON,
			 CASE
					 WHEN B.ACTUAL_END_DATE IS NOT NULL THEN
						'已完成'
					 WHEN (NVL(B.ACTUAL_END_DATE, SYSDATE) - B.PLAN_END_DATE) > 0 AND
...
LEFT   JOIN (SELECT FEEDBACK_NODE_ORIGINAL_ID,
										SUM(CASE
										    --过程反馈数量
														WHEN FEEDBACK_TYPE = 'PROCESS' and APPROVAL_STATUS = '已审核' THEN
														 1
														ELSE
														 0
												END) AS PROCESS_FEED_BACK_CON,
										SUM(CASE
										    --完成反馈数量
														WHEN FEEDBACK_TYPE = 'CARRY_OUT' and APPROVAL_STATUS = '已审核' THEN
														 1
														ELSE
														 0
												END) AS FINISH_FEED_BACK_CON,
										SUM(CASE
										    --到期未完成反馈数量
														WHEN FEEDBACK_TYPE = 'EXPIRED' and APPROVAL_STATUS = '已审核' THEN
														 1
														ELSE
														 0
												END) AS OVERDUE_FEED_BACK_CON
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE VIEW "V_SYS_PROJECT" ("ORG_ID", "ORG_NAME", "COMPANY_NAME", "ORG_TYPE", "PARENT_ID", "CITY_NAME", "PROJ_CODE", "IS_END", "FULL_NAME", "SN", "COMPANY_ID", "PROJECT_ID") AS SELECT
    ...
    || '.'
    || replace(lpad(a.order_code, 3), ' ', '0') AS sn,
...
from (
  ...
  union
  SELECT ID as orgId, STATION_NAME  as orgName ,org_id as parentId, '2' as orgType,0 as isCompany, ORDER_CODE as orderCode,
  ...
  SELECT a.ID as orgId, a.USER_NAME as orgName, b.STATION_ID as parentId, '4' as orgType,0 as isCompany,
         b.ORDER_CODE as orderCode,'001' as orderHierarchyCode
  from SYS_USER a
