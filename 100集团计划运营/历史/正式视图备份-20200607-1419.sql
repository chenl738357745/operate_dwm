...
--------------------------------------------------------
  ...
  CREATE OR REPLACE VIEW "V_POM_PJPLAN_EXM_SOCRE" ("RANK", "ID", "PARENT_ID", "ORG_NAME", "PLANYEAR", "PLANMOTH", "PLANQUATOR", "NODESUM", "ISFINISHTOTAL", "FINISHEXMSOCRE", "SUMNODESCORE", "CTYPE", "PLAN_TYPE") AS SELECT ROW_NUMBER() OVER(PARTITION BY A.PARENT_ID ORDER BY ROUND(CASE
					 WHEN NVL(B.NODESUM,
										0) > 0 THEN
						NVL(ISFINISH,
								0) /
					 ...
					 ELSE
						0
			 END * 100, 2) DESC) AS RANK, A.ID, A.PARENT_ID, A.ORG_NAME, PLANYEAR,
			 PLANMOTH, PLANQUATOR, NVL(B.NODESUM, 0) AS NODESUM,
			 NVL(ISFINISH, 0) AS ISFINISHTOTAL,
			 NVL(FINISHEXMSOCRE, 0) AS FINISHEXMSOCRE,
			 NVL(SUMNODESCORE, 0) AS SUMNODESCORE, 'TYPE_ORG' AS CTYPE,
			 '关键节点计划' AS PLAN_TYPE
FROM   SYS_BUSINESS_UNIT A
LEFT   JOIN (SELECT AB.ID, AB.PARENT_ID, PLANYEAR, PLANMOTH, PLANQUATOR,
										SUM(AB.NODESUM) AS NODESUM, SUM(AB.ISFINISH) AS ISFINISH,
										SUM(AB.FINISHEXMSOCRE) AS FINISHEXMSOCRE,
						 ...
						 FROM   (
											...
											SELECT NVL(B.NODESUM, 0) AS NODESUM,
															NVL(B.ISFINISH, 0) AS ISFINISH,
															NVL(B.FINISHEXMSOCRE, 0) AS FINISHEXMSOCRE,
															NVL(B.SUMNODESCORE, 0) AS SUMNODESCORE, ORG_NAME,
															PLANYEAR, PLANMOTH, PLANQUATOR,
															CONNECT_BY_ROOT ID AS ID,
															CONNECT_BY_ROOT PARENT_ID AS PARENT_ID
											FROM   SYS_BUSINESS_UNIT A
											LEFT   JOIN (
																	 ...
																	 SELECT B1.UNIT_ID, COUNT(A1.ID) AS "NODESUM",
																					 TO_CHAR(PLAN_END_DATE, 'YYYY') AS "PLANYEAR",
																					 CASE
																							 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN
																										'12-26' AND '12-31' OR
																										TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN
																										'01-01' AND '03-25' THEN
																								1
																							 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN
																										'03-26' AND '06-25' THEN
																								2
																							 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN
																										'06-26' AND '09-25' THEN
																								3
																							 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN
																										'09-26' AND '12-25' THEN
																								4
																					 END AS "PLANQUATOR",
																					 CASE
																							 WHEN CASE
																												WHEN TO_CHAR(PLAN_END_DATE, 'DD') > 25 THEN
																												 TO_NUMBER(TO_CHAR(PLAN_END_DATE,
																																					 'MM')) + 1
																												ELSE
																												 TO_NUMBER(TO_CHAR(PLAN_END_DATE,
																																					 'MM'))
																					 ...
																					 SUM((CASE
																							 ...
																							 --延时完成
																									 WHEN TO_CHAR(A1.ACTUAL_END_DATE,
																																'YYYY-MM-DD') IS NOT NULL AND
																												(TRUNC(D.APPROVAL_TIME, 'DD') -
																												 TRUNC(A1.PLAN_END_DATE, 'DD')) BETWEEN 0 AND 4 AND
																												A1.NODE_LEVEL IN
																												('里程碑', '一级节点') THEN
																										A1.STANDARD_SCORE * 0.6
																									 WHEN TO_CHAR(A1.ACTUAL_END_DATE,
																																'YYYY-MM-DD') IS NOT NULL AND
																												(TRUNC(D.APPROVAL_TIME, 'DD') -
																												 TRUNC(A1.PLAN_END_DATE, 'DD')) BETWEEN 0 AND 3 AND
																												A1.NODE_LEVEL IN
																												('二级节点', '三级节点') THEN
																										A1.STANDARD_SCORE * 1.0
																									 WHEN TO_CHAR(A1.ACTUAL_END_DATE,
											...
											WHERE  NVL(B.NODESUM, 0) > 0
											CONNECT BY PRIOR A.ID = A.PARENT_ID) AB
						 GROUP  BY AB.ID, AB.PARENT_ID, PLANYEAR, PLANMOTH, PLANQUATOR) B
ON     A.ID = B.ID
WHERE  A.ORG_TYPE = 0 AND
			 NVL(B.NODESUM, 0) > 0
...
SELECT ROW_NUMBER() OVER(ORDER BY NVL(B.FINISHEXMSOCRE, 0) DESC) AS RANK,
			 CONNECT_BY_ROOT ID AS ID, CONNECT_BY_ROOT PARENT_ID AS PARENT_ID,
			 CONNECT_BY_ROOT ORG_NAME || '-' || PROJECT_NAME AS PROJECT_NAME,
			 PLANYEAR, PLANMOTH, PLANQUATOR, NVL(B.NODESUM, 0) AS NODESUM,
			 NVL(B.ISFINISH, 0) AS ISFINISH,
			 NVL(B.FINISHEXMSOCRE, 0) AS FINISHEXMSOCRE,
			 NVL(B.SUMNODESCORE, 0) AS SUMNODESCORE, 'TYPE_PROJTOP' AS CTYPE,
			 '关键节点计划' AS PLAN_TYPE
FROM   SYS_BUSINESS_UNIT A
LEFT   JOIN (

						 SELECT B1.UNIT_ID, B1.ID AS PORJID

										, TO_CHAR(PLAN_END_DATE, 'YYYY') AS "PLANYEAR",
										 CASE
												 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN '12-26' AND
															'12-31' OR TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN
															'01-01' AND '03-25' THEN
													1
												 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN '03-26' AND
															'06-25' THEN
													2
												 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN '06-26' AND
															'09-25' THEN
													3
												 WHEN TO_CHAR(PLAN_END_DATE, 'MM-DD') BETWEEN '09-26' AND
															'12-25' THEN
													4
										 END AS "PLANQUATOR",
										 CASE
												 WHEN CASE
																	WHEN TO_CHAR(PLAN_END_DATE, 'DD') > 25 THEN
																	 TO_NUMBER(TO_CHAR(PLAN_END_DATE, 'MM')) + 1
																	ELSE
																	 TO_NUMBER(TO_CHAR(PLAN_END_DATE, 'MM'))
															END > 12 THEN
													1
												 ELSE
													CASE
															...
															ELSE
															 TO_NUMBER(TO_CHAR(PLAN_END_DATE, 'MM'))
													END
										 END AS "PLANMOTH", B1.PROJECT_NAME AS PROJECT_NAME,
										 COUNT(A1.ID) AS "NODESUM",
										 SUM(STANDARD_SCORE) AS "SUMNODESCORE"
										 --完成规则
										 ,
										 SUM((CASE
														 WHEN TO_CHAR(A1.ACTUAL_END_DATE, 'YYYY-MM-DD') IS NOT NULL THEN
															1
														 ELSE
															0
												 END)) AS "ISFINISH"
						 ...
						 --判断本月
						 --  AND TO_CHAR(A1.PLAN_END_DATE,'YYYY-MM-DD') BETWEEN
						 --  (SELECT      A.铁建_本年||'-'||TO_CHAR( TO_NUMBER(铁建_本月)-1)||'-'||'26'  FROM V_POM_GETEXAMINE_MOTH A)  AND
						 --  (SELECT    A.铁建_本年||'-'||铁建_本月||'-'||'25' FROM V_POM_GETEXAMINE_MOTH A)
						 --WHERE  B1.UNIT_ID='1b922b32-928c-44a7-85d7-ee12b74f97e0'

						 GROUP  BY B1.UNIT_ID, B1.ID, B1.PROJECT_NAME,
												TO_CHAR(PLAN_END_DATE, 'YYYY'),
												CASE
														WHEN CASE
																		 WHEN TO_CHAR(PLAN_END_DATE, 'DD') > 25 THEN
...
ON     A.ID = B.UNIT_ID
WHERE  NVL(B.NODESUM, 0) > 0 --AND CONNECT_BY_ROOT COMPANY_TYPE=5
CONNECT BY PRIOR A.ID = A.PARENT_ID

----分期汇总

UNION ALL
SELECT ROW_NUMBER() OVER(PARTITION BY B2.PARENT_ID ORDER BY ROUND(CASE
					 WHEN NVL(B2.NODESUM, 0) > 0 THEN
						NVL(ISFINISH, 0) / NVL(B2.NODESUM, 0)
					 ELSE
						0
			 END * 100, 2) DESC) AS RANK, B2.ID, B2.PARENT_ID, B2.PROJSTAGE_NAME,
			 PLANYEAR, PLANMOTH, PLANQUATOR, NVL(B2.NODESUM, 0) AS NODESUM,
			 NVL(ISFINISH, 0) AS ISFINISHTOTAL,
			 NVL(FINISHEXMSOCRE, 0) AS ONTIMEFINISHTOTAL,
			 NVL(SUMNODESCORE, 0) AS SUMNODESCORE, 'TYPE_PROJLAST' AS CTYPE,
			 '关键节点计划' AS PLAN_TYPE
FROM   (SELECT C1.PROJECT_ID AS PARENT_ID, C1.ID,
										...
										ELSE
										 CASE
												 ...
												 ELSE
													TO_NUMBER(TO_CHAR(PLAN_END_DATE, 'MM'))
										 END
								END AS "PLANMOTH", SUM(STANDARD_SCORE) AS "SUMNODESCORE"
								--完成规则
								,
								SUM((CASE
												WHEN TO_CHAR(A1.ACTUAL_END_DATE, 'YYYY-MM-DD') IS NOT NULL THEN
				 ...
				 --WHERE  B1.UNIT_ID='1b922b32-928c-44a7-85d7-ee12b74f97e0'
				 GROUP  BY C1.PROJECT_ID, C1.ID, PROJECT_NAME, STAGE_NAME,
									 TO_CHAR(PLAN_END_DATE, 'YYYY'),
									 CASE
											 WHEN CASE
																WHEN TO_CHAR(PLAN_END_DATE, 'DD') > 25 THEN
...
SELECT ROW_NUMBER() OVER(PARTITION BY A.PARENT_ID ORDER BY ROUND(CASE
					 WHEN NVL(B.NODESUM, 0) > 0 THEN
						NVL(ISFINISH, 0) / NVL(B.NODESUM, 0)
					 ELSE
						0
			 END * 100, 2) DESC) AS RANK, A.ID, A.PARENT_ID, A.ORG_NAME, PLANYEAR,
			 PLANMOTH, PLANQUATOR, NVL(B.NODESUM, 0) AS NODESUM,
...
(
SELECT 
A2.ORGNAME AS ORG_NAME ,A2.ORGID,A2.PLANYEAR,A2.PLANQUATOR,A2.PLANMOTH
...
WHERE  A.PLAN_TYPE='项目主项计划' AND  A.APPROVAL_STATUS='已审核' AND  A1.IS_DELETE=0  
AND   A.COMPANY_NAME  NOT  in(  '中南公司','贵州公司')
) A2

GROUP BY A2.ORGID,A2.ORGNAME,A2.PLANYEAR,A2.PLANQUATOR,A2.PLANMOTH
...
SELECT 
A2.PROJECT_NAME AS PROJECT_NAME,A2.SN ,A2.ORGID,  A2.PROJECTID ,A2.PLANYEAR,A2.PLANQUATOR,A2.PLANMOTH
,COUNT(A2.NODEID) AS 应完成
,SUM(CASE WHEN A2.ACTUAL_END_DATE IS NOT NULL THEN 1 ELSE 0   END ) AS 已完成
,SUM(A2.STANDARD_SCORE) AS SUMNODESCORE
,SUM(NVL(A2.EXAMINATION_SCORE,0)) AS 已完成总分
...
GROUP BY A2.PROJECT_NAME  ,A2.SN ,A2.ORGID,  A2.PROJECTID ,A2.PLANYEAR,A2.PLANQUATOR,A2.PLANMOTH
)A3  ON A3.ORGID=A4.ID
WHERE 1=1
AND A3.应完成>0 
...
AND TO_CHAR(A1.PLAN_END_DATE,'YYYY-MM-DD')
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE VIEW "V_SYS_PROJECT" ("ORG_ID", "ORG_NAME", "ORG_TYPE", "PARENT_ID", "CITY_NAME", "PROJ_CODE", "IS_END", "FULL_NAME", "SN", "COMPANY_ID", "PROJECT_ID") AS SELECT
        a.id              AS org_id,
