...
DROP PROCEDURE "P_OPM_ED_PROJ_CHENL";
...
/
--------------------------------------------------------
--  DDL for Procedure P_OPM_ED_PROJ_CHENL
--------------------------------------------------------
set define off;
...
BEGIN
 ...
 SELECT 'select ''主键'' object_name,'|| wm_concat ('''' || "id" || '''') || ' from dual
union all
select ''土地成本'' object_name,'|| wm_concat ('''' || "kf_land_cost" || '''') || ' from dual
union all
select ''其中：土地出让金（或土地收购价）'' object_name,'|| wm_concat ('''' || "kf_land_transfer_fee" || '''') || ' from dual
union all
select ''其中：补交地价款'' object_name,'|| wm_concat ('''' || "kf_land_payment" || '''') || ' from dual
union all
select ''其中：大市政配套费'' object_name,'|| wm_concat ('''' || "kf_land_municipal_support" || '''') || ' from dual
union all
select ''其中：契税'' object_name,'|| wm_concat ('''' || "kf_land_deed_tax" || '''') || ' from dual
union all
select ''其中：拆迁补偿费、土地熟化费用'' object_name,'|| wm_concat ('''' || "kf_land_compensation" || '''') || ' from dual
union all
select ''其中：其他'' object_name,'|| wm_concat ('''' || "kf_land_other" || '''') || ' from dual
union all
select ''政策性收费'' object_name,'|| wm_concat ('''' || "kf_policy_charge" || '''') || ' from dual
union all
select ''前期工程费'' object_name,'|| wm_concat ('''' || "kf_proj_costs" || '''') || ' from dual
union all
select ''基础设施费'' object_name,'|| wm_concat ('''' || "kf_facil_fee" || '''') || ' from dual
union all
select ''园林绿化道路'' object_name,'|| wm_concat ('''' || "kf_landscap_road" || '''') || ' from dual
union all
select ''建安工程费'' object_name,'|| wm_concat ('''' || "ja_proj_costs" || '''') || ' from dual
union all
select ''其中：基础工程'' object_name,'|| wm_concat ('''' || "ja_basic_proj" || '''') || ' from dual
union all
select ''其中：地下结构'' object_name,'|| wm_concat ('''' || "ja_underground_structure" || '''') || ' from dual
union all
select ''其中：地上结构'' object_name,'|| wm_concat ('''' || "ja_ontheground_structure" || '''') || ' from dual
union all
select ''其中：外装'' object_name,'|| wm_concat ('''' || "ja_exterior" || '''') || ' from dual
union all
select ''其中：初装修'' object_name,'|| wm_concat ('''' || "ja_initial_decoration" || '''') || ' from dual
union all
select ''其中：公区精装修'' object_name,'|| wm_concat ('''' || "ja_public_exquisite" || '''') || ' from dual
union all
select ''其中：户内精装修'' object_name,'|| wm_concat ('''' || "ja_indoor_exquisite" || '''') || ' from dual
union all
select ''其中：给排水'' object_name,'|| wm_concat ('''' || "ja_drainage" || '''') || ' from dual
union all
select ''其中：强电'' object_name,'|| wm_concat ('''' || "ja_strong_current" || '''') || ' from dual
union all
select ''其中：采暖'' object_name,'|| wm_concat ('''' || "ja_heating" || '''') || ' from dual
union all
select ''其中：通风空调'' object_name,'|| wm_concat ('''' || "ja_airiness" || '''') || ' from dual
union all
select ''其中：消防'' object_name,'|| wm_concat ('''' || "ja_fire_control" || '''') || ' from dual
union all
select ''其中：电梯'' object_name,'|| wm_concat ('''' || "ja_elevator" || '''') || ' from dual
union all
select ''其中：弱电及智能化'' object_name,'|| wm_concat ('''' || "ja_weak_current" || '''') || ' from dual
union all
select ''其中：其他'' object_name,'|| wm_concat ('''' || "ja_other" || '''') || ' from dual
union all
select ''公共配套建设费'' object_name,'|| wm_concat ('''' || "pc_support_construct_fee" || '''') || ' from dual
union all
select ''开发间接费'' object_name,'|| wm_concat ('''' || "pc_development_overhead" || '''') || ' from dual
union all
select ''其中：物业补贴'' object_name,'|| wm_concat ('''' || "pc_property_subsidy" || '''') || ' from dual
union all
select ''其中：品质提升基金、科研费用'' object_name,'|| wm_concat ('''' || "pc_resear_expenses" || '''') || ' from dual
union all
select ''其中：其他'' object_name,'|| wm_concat ('''' || "pc_other" || '''') || ' from dual
union all
select ''销售费用'' object_name,'|| wm_concat ('''' || "sa_sales_expense" || '''') || ' from dual
union all
select ''管理费用'' object_name,'|| wm_concat ('''' || "sa_manage_expense" || '''') || ' from dual
union all
select ''财务费用'' object_name,'|| wm_concat ('''' || "sa_finance_expense" || '''') || ' from dual
union all
select ''其中：资本化利息'' object_name,'|| wm_concat ('''' || "sa_capitali_interest" || '''') || ' from dual
union all
select ''其中：费用化利息'' object_name,'|| wm_concat ('''' || "sa_expensed_interest" || '''') || ' from dual
union all
select ''其中：其他'' object_name,'|| wm_concat ('''' || "sa_other" || '''') || ' from dual
union all
select ''增值税'' object_name,'|| wm_concat ('''' || "ld_the_tax" || '''') || ' from dual
union all
select ''城市维护建设税、教育费附加及地方教育费附加'' object_name,'|| wm_concat ('''' || "ld_urban_construc_tax" || '''') || ' from dual
union all
select ''土地增值税（预缴）'' object_name,'|| wm_concat ('''' || "ld_advance" || '''') || ' from dual
union all
select ''土地增值税（清算后补缴）'' object_name,'|| wm_concat ('''' || "ld_payment" || '''') || ' from dual
union all
select ''土地使用税'' object_name,'|| wm_concat ('''' || "ld_land_use_tax" || '''') || ' from dual
union all
select ''印花税（含土地印花等）'' object_name,'|| wm_concat ('''' || "ld_stamp_duty" || '''') || ' from dual
union all
select ''其他'' object_name,'|| wm_concat ('''' || "ld_other" || '''') || ' from dual
union all
select ''总投资（所得税前成本）'' object_name,'|| wm_concat ('''' || "total_investment" || '''') || ' from dual
union all
select ''其中：可控成本（所得税前成本-税金及附加-土地出让金-补交地价款-契税）'' object_name,'|| wm_concat ('''' || "controllable_costs" || '''') || ' from dual
...
BEGIN
    ...
    belong_proj_id as "belong_proj_id", ---所属项目ID
    delivery_date as "belong_proj_id", ---项目交付时间
