...
-----公司id不为空，项目id不为空 根据项目id过滤
when (companyid is not null and projid is not null) then ' left join (select '''||projid||''' as projId from dual) wh on 节点所属项目id=wh.projId '
else '' end;
where_auth_c_sql:=case when (companyid is not null or projid is not null) then ' and wh.projId is not null ' else '' end;
