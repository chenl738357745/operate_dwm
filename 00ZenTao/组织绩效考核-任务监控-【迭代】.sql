...
,CONCAT(备注,';',action.过程备注) as "备注描述"
 FROM (
 /*zt_task*/
SELECT zt_task.id,zt_task.id AS "禅道编号"
...
,zt_task.estimate as '项目预计工时'
,cast(zt_task.consumed as decimal(16,2)) as "实际消耗工时"
,zt_task.`left` AS "实际预估剩余工时"
...
LEFT JOIN (
	...
	LEFT JOIN zt_task as f on result.id=f.id
	 where 1=1 and f.id is null
