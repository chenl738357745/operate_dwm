...
BEGIN--=============================================================页面
-- 删除表页面数据再新增
	DELETE 	FROM		udp_page 	WHERE		CREATOR = d_createName;
	commit;
-->提交数据
--新增页面数据
	INSERT INTO udp_page ( ID, layout_template_id, CREATOR )VALUES( d_functionId, d_layout_template_id, d_createName );
--=============================================================组件实例布局
	DELETE 	FROM		UDP_LAYOUT 	WHERE		CREATOR = d_createName;	commit;
--table
	INSERT INTO udp_layout ( page_id, layout_template_id, component_id, layout_area_template_id, CREATOR )
	VALUES		( d_functionId, d_layout_template_id, d_table1, d_view, d_createName );
		commit;
--=============================================================页面组件
	DELETE 	FROM		udp_component 	WHERE		CREATOR = d_createName;
	commit;
-->提交数据
---table
	...
	VALUES		( d_table1, d_functionId, d_table_dataSource, NULL, NULL, 0, 'TableList1', '', 'TemplateTreetable', '', d_createName );
	

--=============================================================table组件属性
	DELETE 	FROM		udp_component_table 	WHERE		CREATOR = d_createName;	commit;-->提交数据
	DELETE 	FROM		udp_component_table_column 	WHERE		CREATOR = d_createName;	commit;-->提交数据
	INSERT INTO udp_component_table ( ID, detail_url, CREATOR )	VALUES
